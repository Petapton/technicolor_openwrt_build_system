...
void *kerSysGetProzoneMemory(void)
{
    ...
    if (ProzoneMemory_phys)
        return ProzoneMemory_phys;

    if( BcmMemReserveGetByName(TCH_RESERVED_BASE_ADDR_STR, &virt, &size) == 0 ) {
#if defined(CONFIG_ARM64) || defined(CONFIG_ARM)
        phys = virt_to_phys(virt);
#else
        phys = VIRT_TO_PHYS(virt);
#endif
        return (void *)phys + TCH_RESERVED_TOPMEM-PROZONE_RESERVED_MEM;
    } else
        return NULL;
}
