...
static u8 select_channel(void)
{
	...
	return (chan_idx % iproc_priv.num_chan);
}
...
static int aead_enqueue(struct aead_request *req, bool is_encrypt)
{
	...
	struct iproc_ctx_s *ctx = crypto_aead_ctx(crypto_aead_reqtfm(req));
	int err;
	int iv_len = crypto_aead_ivsize(crypto_aead_reqtfm(req));
...
static int aead_givencrypt(struct aead_givcrypt_request *greq)
{
	...
	struct iproc_ctx_s *ctx =
	    crypto_aead_ctx(crypto_aead_reqtfm(&greq->areq));
...
static int spu_dt_read(struct platform_device *pdev)
{
	...
	/* Get number of SPU channels from device tree */
	...
	if (!dt_val_ptr) {
		...
		iproc_priv.num_chan = be32_to_cpup(dt_val_ptr);
		dev_dbg(dev, "Device has %d SPU channels",
			 iproc_priv.num_chan);
	}
...
int iproc_crypto_probe(struct platform_device *pdev)
{
	...
	atomic_set(&iproc_priv.next_chan, (int) iproc_priv.num_chan);
