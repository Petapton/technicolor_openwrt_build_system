...
int bcm_mcast_igmp_admission_process(bcm_mcast_ifdata *pif, int packet_index, int admitted)
{
   ...
   if ( skb )
   {
      if (true == admitted)
      {
         ...
         if(br_bcm_mcast_flood_forward(pif->dev, skb))
            kfree_skb(skb);
      }
      else
      {
                                  ...
                                  int pkt_type)
{
   ...
   hlist_for_each_entry(dst, head, hlist)
   {
      ...
      if (is_routed)
      {
         /* For Host generated mcast packet, the skb->pkt_type is set to
            PACKET_HOST. Forward such packets as long as a snooping entry
            exists for the LAN device regardless the snoop entry type is
            BCM_MCAST_IF_ROUTED or BCM_MCAST_IF_BRIDGED */
