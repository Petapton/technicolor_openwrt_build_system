...
#if !defined(CONFIG_BCM_TCH_BL) // TCH
...
#endif
#if !defined(CONFIG_BCM_TCH_BL) // TCH IF0 #5
...
void kerSysEarlyFlashInit( void )
{
    ...
    memset(inMemNvramData_buf, 0, sizeof(NVRAM_DATA));
    strncpy(inMemNvramData.szBoardId, tch_board, sizeof(inMemNvramData.szBoardId) - 1);
    inMemNvramData.ulNumMacAddrs = 10;
    memcpy(inMemNvramData.ucaBaseMacAddr, "\x02\x10\x18\x88\x68\x01", 6);
    inMemNvramData.backupPsi = 0;
    inMemNvramData.ulSyslogSize = 0;

#if defined(CONFIG_TCH_KF_VOICE_DECT_MASK_ENBITS)
    inMemNvramData.ulBoardStuffOption |= (unsigned int)CONFIG_TCH_KF_VOICE_DECT_MASK_ENBITS;
#endif

    strncpy(inMemNvramData.szVoiceBoardId, tch_board, sizeof(inMemNvramData.szVoiceBoardId) - 1);
    ...
    // use the tch_board variable for both the boardId and voiceBoardId
    strncpy(inMemNvramData.szBoardId, tch_board, sizeof(inMemNvramData.szBoardId) - 1);
    strncpy(inMemNvramData.szVoiceBoardId, tch_board, sizeof(inMemNvramData.szVoiceBoardId) - 1);
#endif
...
#if !defined(CONFIG_BCM_TCH_BL) // TCH IF0 #6
/***********************************************************************
...
#if !defined(CONFIG_BCM_TCH_BL) // TCH IF0 #7
...
#if !defined(CONFIG_BCM_TCH_BL) // TCH IF0 #8
...
#if !defined(CONFIG_BCM_TCH_BL) // TCH IF0 #9
...
#endif // defined(CONFIG_TECHNICOLOR_GPON_PATCH)
...
#if !defined(CONFIG_BCM_TCH_BL) //TCH IF0 10 
...
#if defined(CONFIG_BCM_TCH_BL) || defined(CONFIG_TECHNICOLOR_GPON_PATCH) // TCH
int kerSysScratchPadGet(char *tokenId, char *tokBuf, int bufLen) {
#if defined(CONFIG_TECHNICOLOR_GPON_PATCH)
    if (strncmp(tokenId, RDPA_WAN_TYPE_PSP_KEY, TOKEN_NAME_LEN) == 0)
    {
        memcpy(tokBuf, "GPON", 4);
        return 4;
    }
    else if (strncmp(tokenId, RDPA_WAN_RATE_PSP_KEY, TOKEN_NAME_LEN) == 0)
    {
        memcpy(tokBuf, "0101", 4);
        return 4;
    }
    else return 0;
#else
    printk("kerSysScratchPadGet called!");
    return 0;
#endif
}

int kerSysScratchPadSet(char *tokenId, char *tokBuf, int bufLen) {
    printk("kerSysScratchPadSet called!");
    return 0;
}
