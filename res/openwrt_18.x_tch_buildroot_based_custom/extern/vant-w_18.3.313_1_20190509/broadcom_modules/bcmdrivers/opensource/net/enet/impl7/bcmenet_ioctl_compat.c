...
int enet_ioctl_compat(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    case SIOCGQUERYNUMPORTS:
        {
            uint32_t ports_list = 0; /* XXX: Add crossbar support */

            if (copy_to_user(rq->ifr_data, &ports_list, sizeof(ports_list)))
                return -EFAULT;
        ...
        case ETHSWPHYMODE:
            {
                ...
                else if (ethswctl->type == TYPE_GET)
                {
                    ...
                    ethswctl->phycfg = 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_AUTONEG) ? PHY_CFG_AUTO_NEGO : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_10_HALF) ? PHY_CFG_10HD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_10_FULL) ? PHY_CFG_10FD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_100_HALF) ? PHY_CFG_100HD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_100_FULL) ? PHY_CFG_100FD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_1000_HALF) ? PHY_CFG_1000HD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_1000_FULL) ? PHY_CFG_1000FD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_2500) ? PHY_CFG_2500FD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_5000) ? PHY_CFG_5000FD : 0;
                    ethswctl->phycfg |= (caps & PHY_CAP_10000) ? PHY_CFG_10000FD : 0;
