...
extern int bcmxtmrt_pppoatm_send(struct ppp_channel *pChan,struct sk_buff *skb);
...
static int bcmxtmrt_change_mtu(struct net_device *dev, int new_mtu)
{
    ...
    case TYPE_ETH:
        ...
        if (pDevCtx->ulHdrType == HT_PTM && new_mtu > PTM_MAX_MTU)
          return -EINVAL;
...
/* TCH Modification : Keep the default MTU (1500) set in ether_setup */
#if 0
         dev->mtu = RFC1626_MTU;
#endif
/* End TCH Modification */
      ...
      // TCH modification - initialize device in "not running" state
                    ...
                    PXTMRT_LINK_STATUS_CHANGE pLsc, UINT32 ulDevId)
{
   ...
   if (pDevCtx->ulLinkState != pLsc->ulLinkState)
   {
      ...
      if (pGi->ulDrvState == XTMRT_INITIALIZED)
      {
         ...
         */
...
#if defined(CONFIG_BCM_FAP) || defined(CONFIG_BCM_FAP_MODULE)
         {
             ...
             for (c = 0; c < MAX_RECEIVE_QUEUES; c++)
             {
                 rxdma = pGi->rxdma[c];
                 ...
                 BCM_XTM_DEBUG("Enable XTM receive path, dma channel %d\n", c);
             }
         }
...
#else
         {
             ...
             for (c = 0; c < MAX_RECEIVE_QUEUES; c++)
             {
                 ...
                 if (rxdma->pktDmaRxInfo.rxBds)
                 {
                     ...
                     BcmHalInterruptEnable(SAR_RX_INT_ID_BASE + c);
#endif
                     BCM_XTM_DEBUG("Enable XTM receive path, dma channel %d\n", c);
                 }
             }
      ...
      /* TCH Change 2 :
         set carrier to on if it's not already so that the upper layers are aware of it
      */
      if(!netif_carrier_ok(pDevCtx->pDev)) {
                        ...
                        PXTMRT_LINK_STATUS_CHANGE pLsc)
{
   ...
   {
      // TCH change - set carrier to 0 to notify the upper layers the interface is not in running mode anymore
      // we only do it if the whole group is down (which ulLinkDataMask shoudl represent)
      if(netif_carrier_ok(pDevCtx->pDev)) {
