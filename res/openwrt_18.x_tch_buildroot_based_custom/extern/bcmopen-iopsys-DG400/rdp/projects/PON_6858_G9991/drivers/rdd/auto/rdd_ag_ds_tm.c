...
int rdd_ag_ds_tm_g9991_control_sid_table_set(uint32_t _entry, uint32_t bits)
{
    ...
    for(i = 0; i < NUM_OF_RUNNER_CORES; i++)
    {
        if(rdp_core_to_image_map[i] == image_0_runner_image)
        {
            RDD_BYTES_4_BITS_WRITE(bits, (uint8_t *)RDD_G9991_CONTROL_SID_TABLE_PTR(i) + (sizeof(RDD_BYTES_4_DTS) * _entry));
        }
    }
    return BDMF_ERR_OK;
}
...
int rdd_ag_ds_tm_g9991_control_sid_table_get(uint32_t _entry, uint32_t *bits)
{
    ...
    for(i = 0; i < NUM_OF_RUNNER_CORES; i++)
    {
        if(rdp_core_to_image_map[i] == image_0_runner_image)
        {
            RDD_BYTES_4_BITS_READ(*bits, ((uint8_t *)RDD_G9991_CONTROL_SID_TABLE_PTR(i)) + (sizeof(RDD_BYTES_4_DTS) * _entry));
            return BDMF_ERR_OK;
        }
    }
    return BDMF_ERR_INTERNAL;
