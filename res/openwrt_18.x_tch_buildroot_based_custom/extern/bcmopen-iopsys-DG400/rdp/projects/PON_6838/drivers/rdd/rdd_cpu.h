                                                  ...
                                                  uint32_t           skb_pkt_length )
{
    ...
    if ((BL_LILAC_RDD_EMAC_ID_PCI == xi_args->direction.ds.emac_id) && (rdd_cpu_tx_mode_egress_enq == xi_mode) && (rdpa_dir_ds == traffic_dir))
        RDD_CPU_TX_DESCRIPTOR_CORE_PACKET_LENGTH_L_WRITE ( cpu_tx_descriptor, ( rdd_host_buffer == buffer_type ) ? skb_pkt_length  : ( ( bdmf_pbuf_t * )buffer )->length );
    else
        RDD_CPU_TX_DESCRIPTOR_CORE_PACKET_LENGTH_L_WRITE ( cpu_tx_descriptor, ( rdd_host_buffer == buffer_type ) ? skb_pkt_length + 4 : ( ( bdmf_pbuf_t * )buffer )->length + 4 );
#else
