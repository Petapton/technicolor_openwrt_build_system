...
static inline uint8_t p_rdd_bridge_port_to_mac_context_bridge_port ( BL_LILAC_RDD_BRIDGE_PORT_DTE        xi_bridge_port,
                                                                     BL_LILAC_RDD_WAN_PHYSICAL_PORT_DTE  xi_wan_physical_port )
{
    if ( xi_bridge_port >= BL_LILAC_RDD_MULTICAST_LAN0_BRIDGE_PORT )
    {
        return ( ( xi_bridge_port / BL_LILAC_RDD_MULTICAST_LAN0_BRIDGE_PORT ) | ( LILAC_RDD_MAC_CONTEXT_MULTICAST << 7 ) );
    }
    else
    {
        /* IH forwarding matrix support for quasi bridge port, the IH take the destination port from the context of the MAC lookup */
        if ( xi_bridge_port == BL_LILAC_RDD_WAN_QUASI_BRIDGE_PORT )
        {
            if ( xi_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_FIBER ) 
            {
                return ( BL_LILAC_RDD_WAN_BRIDGE_PORT );
            }
            else if ( xi_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH4 )
            {
                return ( BL_LILAC_RDD_LAN4_BRIDGE_PORT );
            }
            else if ( xi_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH0 )
            {
               return ( BL_LILAC_RDD_LAN0_BRIDGE_PORT );
            }
            else if ( xi_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH1 )
            {
               return ( BL_LILAC_RDD_LAN1_BRIDGE_PORT );
            }
            else if ( xi_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH2 )
            {
               return ( BL_LILAC_RDD_LAN2_BRIDGE_PORT );
            }
            else if ( xi_wan_physical_port == BL_LILAC_RDD_WAN_PHYSICAL_PORT_ETH3 )
            {
               return ( BL_LILAC_RDD_LAN3_BRIDGE_PORT );
            }
        }
...
static inline uint8_t p_rdd_bridge_port_to_iptv_context_bridge_port ( BL_LILAC_RDD_BRIDGE_PORT_DTE  xi_bridge_port )
{
    if ( xi_bridge_port >= BL_LILAC_RDD_MULTICAST_LAN0_BRIDGE_PORT )
    {
        return ( ( xi_bridge_port / BL_LILAC_RDD_MULTICAST_LAN0_BRIDGE_PORT ) | ( LILAC_RDD_MAC_CONTEXT_MULTICAST << 7 ) );
...
static inline BL_LILAC_RDD_BRIDGE_PORT_DTE p_rdd_iptv_context_bridge_port_to_bridge_port ( uint8_t  xi_mac_context_bridge_port )
{
    if ( xi_mac_context_bridge_port & ( LILAC_RDD_MAC_CONTEXT_MULTICAST << 7 ) )
    {
        xi_mac_context_bridge_port &= ~( LILAC_RDD_MAC_CONTEXT_MULTICAST << 7 );
        return ( xi_mac_context_bridge_port << 4 );
    }
    else
    {
        return ( xi_mac_context_bridge_port );
    }
}
...
/* Name:                                                                      */
/*                                                                            */
/*   rdd_iptv_classification_mode_config                                      */
/*                                                                            */
/* Title:                                                                     */
/*                                                                            */
/*   Firmware Driver - Configure iptv filtering mode                          */
/*                                                                            */
/* Abstract:                                                                  */
/*                                                                            */
/*   This function configures the iptv filtering mode. 24 modes are           */
/*   supported:                                                               */
/*     1. destination MAC address                                             */
/*     2. destination MAC address + VLAN id                                   */
/*     3. layer3 destination IP address                                       */
/*     4. layer3 destination IP address + source IP address (SSM)             */
/*                                                                            */
/* Input:                                                                     */
/*                                                                            */
/*   xi_iptv_mode - the selected IPTV filtering mode                          */
/*                                                                            */
/* Output:                                                                    */
/*                                                                            */
...
/* Input:                                                                     */
/*                                                                            */
/*   xi_entry - union containing new entry values. This paremeter can take    */
/*              different forms according to the classification mode.         */
/*                                                                            */
...
/* Name:                                                                      */
/*                                                                            */
/*   rdd_iptv_entry_modify                                                    */
/*                                                                            */
/* Title:                                                                     */
/*                                                                            */
/*   Firmware Driver - Modify an IPTV LUT entry                               */
/*                                                                            */
/* Abstract:                                                                  */
/*                                                                            */
/*   This function modifies an entry whithin the IPTV LUT. The modifiable     */
/*   values are egress bridge port and the wifi ssid vector.                  */
/*                                                                            */
/* Input:                                                                     */
/*                                                                            */
/*   xi_entry_index        - index of the entry in LUT                        */
/*   xi_egress_bridge_port - egress bridge port                               */
...
/* Name:                                                                      */
/*                                                                            */
/*   rdd_iptv_entry_delete                                                    */
/*                                                                            */
/* Title:                                                                     */
/*                                                                            */
/*   Firmware Driver - Delete an IPTV LUT entry                               */
/*                                                                            */
/* Abstract:                                                                  */
/*                                                                            */
/*   This function deletes an IPTV LUT entry.                                 */
/*                                                                            */
/* Input:                                                                     */
/*                                                                            */
/*   xi_entry_index - index of the entry in LUT                               */
...
/* Input:                                                                     */
/*                                                                            */
/*   xi_entry - union containing entry values. This paremeter can take        */
/*              different forms according to the classification mode.         */
/*                                                                            */
/*              1. iptv_lookup_method_mac                                     */
/*				   mac_addr    - search key                                   */
...
/* Output:                                                                    */
/*                                                                            */
/*   xo_entry - union containing retreived entry values. This paremeter can   */
/*              take different forms according to the classification mode.    */
/*                                                                            */
/*              1. iptv_lookup_method_mac                                     */
/*				   mac_addr    - search key                                   */
/*   			   bridge_port - egress port                                  */
...
/* Abstract:                                                                  */
/*                                                                            */
/*   This function retrieves a IPTV LUT entry packet counter                  */
/*                                                                            */
/* Input:                                                                     */
/*                                                                            */
/*   xi_entry_index -  index of the entry in LUT                              */
...
/* Output:                                                                    */
/*                                                                            */
/*   xo_counter_value - entry packet counter                                  */
/*                                      .                                     */
/*                                                                            */
/******************************************************************************/													 
...
/* Name:                                                                      */
/*                                                                            */
/*   rdd_ingress_filters_profile_config 		                              */
/*                                                                            */
/* Title:                                                                     */
/*                                                                            */
/*   Firmware Driver - configure port to profile table                        */
/*                                                                            */
/* Abstract:                                                                  */
/*                                                                            */
/*   This function configures port to profile table                           */
/*		    							                                      */
/*                                                                            */
/* Input:                                                                     */
/*                                                                            */
/*   bridge_port   	- ingress port                                            */
/*   xi_profile_id  - filters profile                                         */
/*                                                                            */
/* Output:                                                                    */
/*   BL_LILAC_RDD_OK - No error                                               */
/*                                      .                                     */
