...
#if defined(RUNNER_COHERENT_RINGS) && !defined(_CFE_) && !defined(RDP_SIM)
#include "rdp_drv_rnr.h"
#include "bcm_map_part.h"
#include "bcm_misc_hw_init.h"
#include "linux/sizes.h"
static uint64_t coherent_base = 0;
...
#if defined(RUNNER_COHERENT_RINGS) && !defined(_CFE_) && !defined(RDP_SIM)
/*  COHERENT RING ALLOCATION
 *
 * in platform where CCI-X00 exits together with UBUS, a decode window must be configured in master
 * which would like to be mapped to CCI on every DDR transaction, in adition cache bit should be set in order
 * to set the cache merge+invalidate function in CCI engine
 *
 *
 * in order to turn all runner rings to coherent, we have to pre-allocate all ring descriptors space as a one chunk
 * of contagious memory so it will be set as "Outer-Sharable" and UBUS window will be defined.
 *
 * the space is distributed among the rings statically in the following layout
 *  |------------------|------------------|----------------|---------------|------------|------------|------------|------------|-------------|
 *  |__________________|__________________|________________|_______________|____________|____________|____________|____________|_____________|
 *  |    Ring Type     |Feed Recycle Ring | RX Recycle Ring| Feed Ring     | Data Ring 0| Data ring 1| Data ring 2| Data Ring .| Data Ring 31|
 *  |------------------|------------------|----------------|---------------|------------|------------|------------|------------|-------------|
 *  |Num Of descriptors|       16k        |       16k      |     32k       |     1k     |     1k     |     1k     |     1k     |     1k      |
 *  |------------------|------------------|----------------|---------------|------------|------------|------------|------------|-------------|
 *  |Descriptor size   |      8 bytes     |     8 bytes    |   8 bytes     |  16 bytes  |  16 bytes  |  16 bytes  |  16 bytes  |  16 bytes   |
 *  |------------------|------------------|----------------|---------------|------------|------------|------------|------------|-------------|
 *
 *  to support 32 data rings we need less then 1Mb of DDR
 *
 * */

#define FEED_RING_SIZE_COHERENT_PURPOSE     (1024 * 32)
#define RECYCLE_RING_SIZE_COHERENT_PURPOSE  (1024 * 16)

#define COHERENT_FEED_RECY_RING_SIZE BCM_DCACHE_ALIGN(RECYCLE_RING_SIZE_COHERENT_PURPOSE * sizeof(CPU_RECYCLE_DESCRIPTOR) \
                                                 + BCM_DCACHE_LINE_LEN + RW_INDEX_SIZE*2)
#define COHERENT_RX_RECY_RING_SIZE BCM_DCACHE_ALIGN(RECYCLE_RING_SIZE_COHERENT_PURPOSE * sizeof(CPU_RECYCLE_DESCRIPTOR) \
                                                 + BCM_DCACHE_LINE_LEN + RW_INDEX_SIZE*2)
#define COHERENT_FEED_RING_SIZE BCM_DCACHE_ALIGN(FEED_RING_SIZE_COHERENT_PURPOSE * sizeof(CPU_FEED_DESCRIPTOR) \
                                                 + BCM_DCACHE_LINE_LEN)
#define COHERENT_DATA_RING_SIZE BCM_DCACHE_ALIGN(RDPA_CPU_QUEUE_MAX_SIZE * sizeof(CPU_RX_DESCRIPTOR) \
                                                 + BCM_DCACHE_LINE_LEN)
uint64_t alloc_coherent_rings(uint8_t ring_type,uint32_t ring_id, uint64_t *phys_addr)
{
    int rc;
    uint32_t   ring_offset = 0;
    uint32_t   alloc_size = SZ_1M;
    int win;
    ...
    if(!coherent_base)
    {
        /*Set runner device as coherent so dma_coherent_alloc will return cached address*/
        rdp_dummy_dev->archdata.dma_coherent = 1;
        coherent_base = (uint64_t)rdp_mm_aligned_alloc(alloc_size, (bdmf_phys_addr_t*)&dma_handle);
        if(!coherent_base)
            BUG();

        rdp_dummy_dev->archdata.dma_coherent = 0;

        bdmf_trace("RDP CPU Rx descriptors working in Coherent Mode 0x%pK size 0x%x is outer sharable\n",
                (void *)coherent_base, alloc_size);

#if !defined(RDP_SIM)
        /*map runner ubus for CCI-400*/
        rc = drv_rnr_quad_ubus_decode_wnd_cfg(win, (uint32_t)dma_handle, log2_32(alloc_size), DECODE_CFG_PID_B53, 1);

        rc = rc ? rc :ubus_master_decode_wnd_cfg(MST_PORT_NODE_RQ0, win, (uint32_t)dma_handle, log2_32(alloc_size), DECODE_CFG_PID_B53, 1);
#if defined (CONFIG_BCM96858) || defined(_BCM96858_) 
        rc = rc ? rc :ubus_master_decode_wnd_cfg(MST_PORT_NODE_RQ1, win, (uint32_t)dma_handle, log2_32(alloc_size), DECODE_CFG_PID_B53, 1);
        rc = rc ? rc :ubus_master_decode_wnd_cfg(MST_PORT_NODE_RQ2, win, (uint32_t)dma_handle, log2_32(alloc_size), DECODE_CFG_PID_B53, 1);
        rc = rc ? rc :ubus_master_decode_wnd_cfg(MST_PORT_NODE_RQ3, win, (uint32_t)dma_handle, log2_32(alloc_size), DECODE_CFG_PID_B53, 1);
#endif
        rc = rc ? rc :ag_drv_rnr_quad_ext_flowctrl_config_token_val_set(0, DECODE_CFG_PID_B53, 8);
        rc = rc ? rc :ubus_master_set_token_credits(MST_PORT_NODE_RQ0, DECODE_CFG_PID_B53, 8);
        if(rc)
        {
           bdmf_trace("%s(%d):Failed to alloc Coherent rings\n",__FUNCTION__, __LINE__);
           return 0;
        }
#endif
    ...
    /*each ring's predefined position in global allocation*/
    switch(ring_type)
    {
        case rdpa_ring_recycle:
            if (ring_id == FEED_RCYCLE_RING_ID)
                ring_offset = 0;
            else
                ring_offset = COHERENT_FEED_RECY_RING_SIZE;
            break;
        case rdpa_ring_feed:
            ring_offset = COHERENT_FEED_RECY_RING_SIZE + COHERENT_RX_RECY_RING_SIZE;
            break;
        case rdpa_ring_data:
            ring_offset = COHERENT_FEED_RECY_RING_SIZE + COHERENT_RX_RECY_RING_SIZE + COHERENT_FEED_RING_SIZE +
                (ring_id * COHERENT_DATA_RING_SIZE);
            break;
        default:
            bdmf_trace("%s(%d):Wrong ring_type %d\n",__FUNCTION__, __LINE__, ring_type);
            return 0;
    }

    *phys_addr = dma_handle + ring_offset;
    return (coherent_base + ring_offset);
}
#endif

static void *rdp_ring_desc_alloc(uint8_t ring_type,uint8_t ring_id, uint32_t size, bdmf_phys_addr_t *phy_addr_p)
{
#if defined(RUNNER_COHERENT_RINGS) && !defined(_CFE_) && !defined(RDP_SIM)
    return (void *)alloc_coherent_rings(ring_type, ring_id, phy_addr_p);
#else
    return rdp_mm_aligned_alloc(size, phy_addr_p);
#endif
}
...
#if !defined(RUNNER_COHERENT_RINGS) && !defined(RDP_SIM)
    if(pDescriptor->base)
    ...
    RING_CB_FUNC* ringCb)
{
    ...
    {
        pDescriptor->base = rdp_ring_desc_alloc(rdpa_ring_recycle, ring_id ,
                                    (pDescriptor->size_of_entry * entries) + RW_INDEX_SIZE*2, &phy_addr);
        ...
        *pDescriptor->write_idx = 0;
        ...
        pDescriptor->base =  rdp_ring_desc_alloc(ring_type, ring_id ,
                            (pDescriptor->size_of_entry * entries), &phy_addr);
...
#else
    INTERN_PRINT("Done initializing Ring %d Base=0x%pK num of entries= %d RDD Base=%lxK descriptor=0x%p\n",
