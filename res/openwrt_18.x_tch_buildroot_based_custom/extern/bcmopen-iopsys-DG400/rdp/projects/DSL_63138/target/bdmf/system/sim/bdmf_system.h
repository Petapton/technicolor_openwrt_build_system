...
#ifdef XRDP
#define NATC_DDR_SEGMENT_SIZE   (8*1024*1024) /* 16MB */
...
static inline void *bdmf_alloc_uncached(int size, bdmf_phys_addr_t *phys_addr_p)
{
    *phys_addr_p = (bdmf_phys_addr_t)(bdmf_alloc(size));
 ...
 * If the function returns NULL, caller is responsible for "data" deallocation
 */
static inline bdmf_sysb bdmf_sysb_header_alloc(bdmf_sysb_type sysb_type, void* datap, uint32_t len, uint32_t context, uint32_t flags)
{
 ...
 */
static inline uint32_t bdmf_sysb_databuf_alloc(void **bufp, uint32_t num_buffs, uint32_t context)
{
