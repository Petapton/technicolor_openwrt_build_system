...
void (*f_rdpa_wan_flag_set)(uint32_t val, uint32_t mask);
EXPORT_SYMBOL(f_rdpa_wan_flag_set);
void rdpa_wan_flag_set(uint32_t val, uint32_t mask)
{
    if (!f_rdpa_wan_flag_set)
    {
        BDMF_TRACE_ERR("rdpa.ko is not loaded\n");
        return;
    }
    return f_rdpa_wan_flag_set(val, mask);
}
EXPORT_SYMBOL(rdpa_wan_flag_set);
