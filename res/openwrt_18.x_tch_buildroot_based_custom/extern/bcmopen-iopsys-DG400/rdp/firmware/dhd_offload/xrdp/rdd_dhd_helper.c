...
{    
    ...
    WMB();
    /* wakeup timer task  */
    return ag_drv_rnr_regs_cfg_cpu_wakeup_set(get_runner_idx(dhd_tx_post_runner_image), IMAGE_1_CPU_IF_1_DHD_TIMER_THREAD_NUMBER);
}
...
{       
    ...
    /* Invalidate lkp entries for specific radio - size of lookup table is 1/3 of total*/
    lkp_tbl_size = RDD_DHD_FLOW_RING_CACHE_LKP_TABLE_SIZE/3;
