...
void _rdd_iptv_linked_list_init(void)
{
    ...
    /* 0 value is treated as error in rdpa */
    g_free_linked_list[0] = IPTV_CTX_ENTRY_IDX_NULL;
    g_head_idx = 1;
    for (i = g_head_idx; i < RDD_IPTV_DDR_CONTEXT_TABLE_SIZE; i++)
    {
...
void _rdd_iptv_ctx_entry_alloc(uint32_t *entry_idx)
{
    ...
    {
        /* head == RDD_IPTV_DDR_CONTEXT_TABLE_SIZE */
        ...
        rdpa_iptv_channel_key_t *key)
{
    ...
    RDD_BTRACE("Ddr_ctx_entry: valid = %d, ip_ver = %d, any_vid = %d, any_src_ip = %d, vid = %d, "
        "next_entry_idx = %d, ssid_vector_0 = 0x%x,  ssid_vector_1 = 0x%x, ssid_vector_2 = 0x%x, "
        ...
        ddr_ctx_entry->any_src_ip, ddr_ctx_entry->vid, ddr_ctx_entry->next_entry_idx, ddr_ctx_entry->ssid_vector_0,
    ...
    RDD_IPTV_DDR_CONTEXT_ENTRY_DTS *ddr_ctx_entry)
{
    ...
    if (replications_num)
    {
        ...
        /* TODO: change 6 (log of 64)  to a number derived from hw data structures */
        ddr_ctx_entry->pool_num = FPM_POOL_ID_LAST -  ddr_ctx_entry->replications / (g_fpm_base_buffer_size >> 6);
    }
    ddr_ctx_entry->wlan_mcast_index = iptv_entry->wlan_mcast_index;
    ddr_ctx_entry->ssid_vector_0 = iptv_entry->wlan_mcast_fwd_table.wfd_0_ssid_vector;
    ...
    /* XXX: Update WLAN TC? */
    for (i = 0; ic_ctx && i < RDD_IPTV_DDR_CONTEXT_ENTRY_RULE_BASED_RESULT_NUMBER; i++)
        ddr_ctx_entry->rule_based_result[i] = ic_ctx[i];
    ...
    rdd_iptv_entry_t *iptv_entry, uint8_t *ic_ctx)
{
    ...
    iptv_entry->wlan_mcast_fwd_table.wfd_0_ssid_vector = ddr_ctx_entry->ssid_vector_0;
    ...
    /* XXX: Update WLAN TC */
    for (i = 0; ic_ctx && i < RDD_IPTV_DDR_CONTEXT_ENTRY_RULE_BASED_RESULT_NUMBER; i++)
        ic_ctx[i] = ddr_ctx_entry->rule_based_result[i];
}
