    ...
    rdp_tcam_key_area_t     *tcam_mask)
{
    ...
    if (rule_key_mask & ~table->all_fields_mask)
        return BDMF_ERR_PARM;
...
void rdd_tcam_ic_result_entry_compose(uint16_t index, const rdd_ic_context_t *ctx, uint8_t *entry)
{
    ...
    RDD_TRACE("ctx values to be used = { priority = %d, tx_flow = %d, egress_port = %d }",
        ctx->priority, ctx->tx_flow, ctx->egress_port);
	rdd_ic_result_entry_compose(index, ctx, entry);
}

void rdd_tcam_debug_cntr_id_cfg(uint16_t index, uint32_t cntr_id)
{
    RDD_BTRACE("index = %d, cntr_id = %d\n", (int)index, cntr_id);
    RDD_IC_EXT_CONTEXT_ENTRY_CNTR_ID_WRITE_G(cntr_id, RDD_IC_EXT_CONTEXT_TABLE_ADDRESS_ARR, index);
}
...
uint32_t rdd_tcam_debug_cntr_id_get(uint16_t index)
{
    ...
    RDD_IC_EXT_CONTEXT_ENTRY_CNTR_ID_READ_G(cntr_id, RDD_IC_EXT_CONTEXT_TABLE_ADDRESS_ARR, index);
    return cntr_id;
}
