...
static int init_ctx(imgif_ctx_t *ctxP)
{
    ...
    if (ctxP->calCrc32Cb != NULL)
        ctxP->calCrc32Cb(0, &ctxP->crc_calc, NULL, 0);
...
static int write_bytes_to_emmc_device( imgif_ctx_t *ctxP, char * data, unsigned int num_bytes )
{
    ...
    while( num_bytes )
    {
        ...
        /* Do crc on non-tag data */
        if( ctxP->current_img_segment != IMGIF_EMMC_IMG_SEGMENT_TAG )
        {
            if (ctxP->calCrc32Cb != NULL)
                ctxP->calCrc32Cb(1, &ctxP->crc_calc, (unsigned char*)datap, bytes_to_write);
        }
        imgif_debug("crc tally:0x%08x\n", ctxP->crc_calc);
...
static void dump_ctx(imgif_ctx_t *ctxP)
{
    ...
    imgif_debug("crc_calc 0x%08x crc_exp 0x%08x\n", ctxP->crc_calc, ctxP->crc_exp);
...
static int finalize_emmc_img_write(imgif_ctx_t *ctxP, UBOOL8 abortFlag)
{
    ...
    /* Check if image CRC matched expected value */
    if( ctxP->crc_calc == ctxP->crc_exp )
    {
#if IMG_WRITE_FS        
    ...
    {
        printf("%s: Image CRC check failed! expected:0x%08x, calculated:0x%08x\n",
                __FUNCTION__,
                ctxP->crc_exp,
                ctxP->crc_calc);
