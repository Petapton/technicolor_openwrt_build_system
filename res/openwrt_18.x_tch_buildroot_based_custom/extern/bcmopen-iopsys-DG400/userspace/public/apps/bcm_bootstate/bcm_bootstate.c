...
int main(int argc, char *argv[])
{
   if (argc != 2)
   {
      ...
      printf( \
          ...
          "%c%cFirst  partition commit flag    : %c\n" \
          "%c%cSecond partition type           : %s\n" \
          "%c%cSecond partition sequence number: %d\n" \
          "%c%cSecond partition commit flag    : %c\n", \
             (boot == 1) ? 'B' : ' ', (reboot == 1) ? '>' : ' ', (seq1 >= 0) ? (!string1 ? "JFFS2/UBI" : "pureUBI/GPT") : "NONE", \
             (boot == 1) ? 'B' : ' ', (reboot == 1) ? '>' : ' ', seq1, \
             (boot == 1) ? 'B' : ' ', (reboot == 1) ? '>' : ' ', string1, \
             (boot == 2) ? 'B' : ' ', (reboot == 2) ? '>' : ' ', (seq2 >= 0) ? (!string2 ? "JFFS2/UBI" : "pureUBI/GPT") : "NONE", \
