...
# Initialize bdmf shell
    ...
    # Mux the control IOs
    sw 0xff80052c "${MUXSELECT:$SIDXSEL:10}"
...
#   echo "r" > /proc/i2c_mux/i2cmuxOper
    ...
    # Enable receivers
    echo "a 0x44 0x86 1 ${RX5TO0:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0x96 1 ${RX11TO6:$SIDX:4}" > /proc/i2c_pon/ponPhyTest

    # Enable equalizers
    echo "a 0x44 0x87 1 ${RX5TO0:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0x97 1 ${RX11TO6:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    
    # Switch to SMA - Write to "next state"
    echo "a 0x44 0xb0 1 ${SWITCHOUT10:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0xb1 1 ${SWITCHOUT32:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0xb2 1 ${SWITCHOUT54:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0xb3 1 ${SWITCHOUT76:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0xb4 1 ${SWITCHOUT98:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0xb5 1 ${SWITCHOUT1110:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    
    # XPT_Update - Write "next state" map to "current state" map (i.e. save changes to map)
    echo "a 0x44 0xdf 1 0x1" > /proc/i2c_pon/ponPhyTest

    # Verify (read) "current state" map
#    echo "a 0x44 0xd0 1" > /proc/i2c_pon/ponPhyTest
#    echo "a 0x44 0xd1 1" > /proc/i2c_pon/ponPhyTest
#    echo "a 0x44 0xd2 1" > /proc/i2c_pon/ponPhyTest
#    echo "a 0x44 0xd3 1" > /proc/i2c_pon/ponPhyTest
#    echo "a 0x44 0xd4 1" > /proc/i2c_pon/ponPhyTest
#    echo "a 0x44 0xd5 1" > /proc/i2c_pon/ponPhyTest

    # Enable transmitters
    echo "a 0x44 0x02 1 ${TX7TO0:$SIDX:4}" > /proc/i2c_pon/ponPhyTest
    echo "a 0x44 0x03 1 ${TX11TO8:$SIDX:4}" > /proc/i2c_pon/ponPhyTest

    #Invert polarity of input to sequoia (transmitter 11, register 0x0B [Tx11to8_Sign])
    echo "a 0x44 0x0b 1 0x08" > /proc/i2c_pon/ponPhyTest

    if [[ $SIDX == 0 ]]
    then
        echo innn!!!!
        #Invert polarity of input to xgpon cage (transmitter 1, register 0x0A [Tx7to0_Sign])
        echo "a 0x44 0x0a 1 0x02" > /proc/i2c_pon/ponPhyTest
    fi
