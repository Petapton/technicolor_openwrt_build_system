...
"-r         --rawoob             Read OOB data in RAW mode (default if noecc)\n"
...
static bool			rawoob = false;		// read oob in raw mode
...
static void process_options(int argc, char * const argv[])
{
	...
	for (;;) {
		int option_index = 0;
		static const char *short_options = "s:f:l:opqncar";
	...
	if (omitoob && rawoob) {
		if (oob_default) {
			...
			fprintf(stderr, "The rawoob and omitoob options are mutually-\n"
