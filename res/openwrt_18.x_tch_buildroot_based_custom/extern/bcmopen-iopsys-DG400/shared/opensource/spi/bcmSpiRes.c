...
#ifndef _CFE_
static DEFINE_SPINLOCK(spi_spinlock);
#ifdef SPI
...
#else
        spin_lock(&spi_spinlock);
        ret = BcmLegSpiRead( msg_buf, msg_buf, prependcnt, nbytes, devId, freqHz );
        spin_unlock(&spi_spinlock);
        return ret;
#endif
    }
...
#else
        spin_lock(&spi_spinlock);
        ret = BcmHsSpiRead( msg_buf, msg_buf, prependcnt, nbytes, devId, freqHz,
...
#else
        spin_lock(&spi_spinlock);
        ret = BcmLegSpiWrite( msg_buf, nbytes, devId, freqHz );
        spin_unlock(&spi_spinlock);
        return ret;
#endif
    }
...
#else
        spin_lock(&spi_spinlock);
        ret = BcmHsSpiWrite( msg_buf, nbytes, devId, freqHz, 
                              bcmSpiCtrlState[devId] );
        spin_unlock(&spi_spinlock);
        return ret;
#endif
...
int BcmSpi_MultibitRead( struct spi_transfer *xfer, int busNum, int devId)
{
    ...
    if ( LEG_SPI_BUS_NUM == busNum )
    {
        ...
        spin_lock(&spi_spinlock);
        ret = BcmLegSpiRead(xfer->tx_buf, xfer->rx_buf, xfer->prepend_cnt, xfer->len, 
                             devId, xfer->speed_hz);
        spin_unlock(&spi_spinlock);
        return ret;
#endif
...
#else
        spin_lock(&spi_spinlock);
        ret = BcmHsSpiMultibitRead( xfer, devId, bcmSpiCtrlState[devId] );
        spin_unlock(&spi_spinlock);
        return ret;
#endif
    }
