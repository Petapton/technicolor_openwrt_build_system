...
static int pdc_work_thread(void *cookie)
{
	...
	while ( 1 ) {
		...
		{
			struct pdc_state *pdcs = &pdcg->pdc_state[0];
			struct brcm_message *msg;

			/* check for received frames and reclaim descriptors */
			msg = pdc_crypto_engine_poll(pdcs);
			if (msg)
			{
				dev_dbg(&pdev->dev, "%s(): invoking client rx cb", __func__);
				msg->rx_callback(msg);
				count++;
			}
			else
			{
				dev_dbg(&pdev->dev, "%s(): no SPU response available", __func__);
				break;
			}
		}
		...
		else
		{
			...
			reg_write32(intstatus, pdcg->pdc_reg_vbase + PDC_INTSTATUS_OFFSET);
			if ( 0 == (intstatus & PDC_INTMASK) )
			{
				/* finished work, enable interrupts and return */
				pdcg->work_avail = 0;
				reg_write32(PDC_INTMASK, pdcg->pdc_reg_vbase + PDC_INTMASK_OFFSET);
			}
		}
	}
	return 0;
...
static int pdc_interrupts_init(struct platform_device *pdev)
{
	...
	reg_write32(PDC_INTMASK, pdcg->pdc_reg_vbase + PDC_INTMASK_OFFSET);
	reg_write32(PDC_LAZY_INT, pdcg->pdc_reg_vbase + PDC_RCVLAZY0_OFFSET);
