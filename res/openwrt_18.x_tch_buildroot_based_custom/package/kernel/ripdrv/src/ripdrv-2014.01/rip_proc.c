...
/**
 * Handler for write system call on the "delete" entry in the RIP proc filesystem.
 * Writing a valid rip2 id will delete an entry from the RIP.
 * \param id RIP entry id
 * \return 1 on success
 */
ssize_t rip_proc_delete(struct file     *filp,
                     const char __user  *buff,
                     size_t             len,
                     loff_t             *ppos)
{
	...
	// check to see if index already deleted!
	if (NULL == rip_proc_find_entry(buffer)) {
		/* Deleting a non existing ID should not result in error, so return len */
		return len;
	}

	tmp = rip_proc_delete_entry(buffer);
	if (tmp == 1) {
		rip2_drv_write_d(id, 1);
	}
...
RIP_SEQ_FOPS_WO(rip_proc_delete_fops, rip_proc_delete);
...
//#endif  /* CONFIG_RIPDRV_ANVIL */
	...
	tmp = rip_proc_add_entry("delete", 0644, &rip_proc_delete_fops, 0);
	if (tmp == NULL) {
		goto out_err;
	}
