...
//    FLASH_READ((size_t)rip2_invalid_idx, sizeof(T_RIP2_ITEM), &ret, &item);
	FLASH_READ(((size_t)ripStart + RIP2_OFFSET + (rip2_invalid_idx)), sizeof(T_RIP2_ITEM), &ret, &item);
    DBG("item with ID 0x%04x at location %x\r\n", id, rip2_invalid_idx);
    DBG("item with ID 0x%04x at location %x\r\n", BETOH16(item.ID), rip2_invalid_idx);
...
/*
 * Updates a rip2 field performing a delete
 *
 * RETURNS: RIP2_SUCCESS if the item was successfully deleted
 *          RIP2_ERR_PERM if the flags don't match or there are no write
 *          permissions
 */
int rip2_drv_write_d(T_RIP2_ID     id,
                     int overwrite)
{
    T_RIP2_ITEM item;
    int         ret = RIP2_ERR_NOELEM;
    ...
    if (RIP2_SUCCESS != rip2_get_addr(id, &idx_addr)) {
        // item does not exist, so exit
        DBG("%s: item 0x%x does not exist\n", __FUNCTION__, id);
        return ret;
    }
    ...
    {
        // item exists, so invalidate it.
        // procedure: make current index item invalid

#if !defined(CONFIG_RIPDRV_ANVIL)
        // Do not overwrite READ-ONLY
        if (!overwrite) {
            if (!(BETOH32(item.attr[ATTR_HI]) & RIP2_ATTR_WRITABLE)) {
                DBG("%s: read only bit is set\n", __FUNCTION__);
                return RIP2_ERR_PERM;
            }
        }
        ...
        LOCK();
        DBG("%s: item.ID = %04x\n", __FUNCTION__, BETOH16(item.ID));
        DBG("%s: item.addr = 0x%04x\n", __FUNCTION__, BETOH32(item.addr));
        DBG("%s: item.attr_hi = 0x%04x\n", __FUNCTION__, BETOH32(item.attr[ATTR_HI]));
        DBG("%s: item.attr_lo = 0x%04x\n", __FUNCTION__, BETOH32(item.attr[ATTR_LO]));
        DBG("%s: item.length = 0x%04x\n", __FUNCTION__, BETOH32(item.length));

        DBG("%s: invalidating index id %04x at address 0x%04x\n", __FUNCTION__, id, idx_addr);

        ret = rip2_invalidate_idx(RIP2_START, idx_addr, id);
        if (ret == RIP2_SUCCESS) {
            DBG("%s: rip2_invalidate_idx successful\n", __FUNCTION__);
        } else {
            DBG("%s: rip2_invalidate_idx failed with ret %d\n", __FUNCTION__, ret);
        }
...
/*
 * Finds the first index item address with the given RIP2 ID and the valid flag set.
 *
 * RETURNS: RIP2_ERR_NOELEM if no matching index found,
 *          RIP2_SUCCESS otherwise
 */
int rip2_get_addr(T_RIP2_ID    id,
                  int  *itemaddr)
{
    T_RIP2_ITEM *from = NULL;
    T_RIP2_ITEM buf;
